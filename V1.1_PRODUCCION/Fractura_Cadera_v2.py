...
if authentication_status:
    ...
    # _________________________________________________ Datos Paciente ______________________________________________
    ...
    # Checkboxes de condiciones adicionales
    vive_centro = st.sidebar.checkbox("Vive en el centro")
    vive_solo = st.sidebar.checkbox("Vive solo")
    reingreso = st.sidebar.checkbox("Reingreso")
    alergia_medicamentosa = st.sidebar.checkbox("Alergia medicamentosa")
    alergia_alimentaria = st.sidebar.checkbox("Alergia alimentaria")
    otras_alergias = st.sidebar.checkbox("Otras alergias")

    # _________________________________________________ Datos Fractura ______________________________________________
    st.sidebar.subheader("Datos de la Fractura")

    # Tipo de fractura
    tipo_fractura = st.sidebar.selectbox(
        "Tipo de Fractura", 
        ["Intracapsular no desplazada", "Intracapuslar desplazada", "Pertrocantérea", "Subtrocantérea", "Otra"], 
        index=2
    )
    ...
    # Lado de fractura
    lado_fractura = st.sidebar.radio("Lado de la fractura", ["Izquierda", "Derecha"], index=0)
    lado_fractura_normal = lado_fractura
    lado_fractura = 0 if lado_fractura == "Izquierda" else 1

    # _________________________________________________ Geriatría ______________________________________________
    st.sidebar.subheader("Datos Geriátricos")

    # Movilidad pre-fractura
    movilidad_pre = st.sidebar.slider("Movilidad Pre-Fractura", 1, 4, 3)
    
    # Riesgo de caída
    riesgo_caida = st.sidebar.slider("Riesgo de caída", 1, 10)

    # Índices funcionales
    barthel = st.sidebar.slider("Barthel", 0, 100)
    braden = st.sidebar.slider("Braden", 8, 23)

    # Medicación y condiciones adicionales
    ...
    # _________________________________________________ Datos Médicos ______________________________________________
    ...
    acido_folico = st.sidebar.checkbox("Ácido fólico")
    hta = st.sidebar.checkbox("HTA")
    anemia = st.sidebar.checkbox("Anemia")
    vitamina_d = st.sidebar.checkbox("Vitamina D")
    obesidad = st.sidebar.checkbox("Obesidad")
    osteoporosis = st.sidebar.checkbox("Osteoporosis")
